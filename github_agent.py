...
github_agent = create_openai_functions_agent(
    github_llm, github_tools, github_prompt)
github_agent_executor = AgentExecutor(
    agent=github_agent, tools=github_tools, verbose=True)


@tool
def call_github_agent(request: str):
    """send a request to the github llm agent."""
